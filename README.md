...
| Location consent flow | `components/WelcomePopup.tsx` and `app/api/location/route.ts` |
...
- Location is optional: browser permission gives more accurate coordinates; when it is denied, the server may save an approximate city/region from the visitor IP through an IP geolocation provider. Visit history is kept in server memory, so it is cleared when the server restarts and is not suitable for durable multi-instance storage.
